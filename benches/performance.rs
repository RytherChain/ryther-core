...
use ryther_core::types::{Address, U256, StateKey, sha256, keccak256, BlsSignature};
...
use ryther_core::crypto::threshold::{ThresholdParams, ShamirSecretSharing, ThresholdEncryptor};
...
fn bench_crypto(c: &mut Criterion) {
    ...
    group.bench_function("threshold_encrypt_256b", |b| {
        b.iter(|| {
            encryptor.encrypt(black_box(&plaintext), black_box(&validators))
        })
    });
...
fn bench_transactions(c: &mut Criterion) {
    ...
    group.bench_function("hash_tx", |b| {
        b.iter(|| sha256(black_box(&tx_bytes)))
    });
