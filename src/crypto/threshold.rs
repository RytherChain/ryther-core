...
impl ThresholdEncryptor {
    ...
    pub fn encrypt(
        ...
        let key_shares: Vec<_> = shares.iter().map(|share| {
    ...
    fn xor_encrypt(&self, plaintext: &[u8], key: &[u8; 32], nonce: &[u8; 12]) -> Vec<u8> {
        ...
        for (i, byte) in plaintext.iter().enumerate() {
            ...
            let key_byte = sha256(&[&keystream_seed[..], &(i as u64).to_le_bytes()[..]].concat())[0];
...
impl DkgSession {
    ...
    pub fn generate_contribution(&mut self) -> Vec<[u8; 32]> {
        ...
        let commitments: Vec<[u8; 32]> = polynomial.iter()
            .map(|coef| sha256(coef))
            .collect();
    ...
    pub fn generate_shares(&self) -> HashMap<ValidatorId, SecretShare> {
        ...
        for (i, validator) in self.validators.iter().enumerate() {
            ...
            shares.insert(validator.clone(), SecretShare {
                index: x,
                value: y,
    ...
    fn evaluate_at(&self, sss: &ShamirSecretSharing, coefficients: &[[u8; 32]], x: u32) -> [u8; 32] {
...
mod tests {
    ...
    fn test_shamir_insufficient_shares() {
        ...
        let shares = vec![
            SecretShare { index: 1, value: [1u8; 32], holder: ValidatorId([0; 48]) },
            SecretShare { index: 2, value: [2u8; 32], holder: ValidatorId([1; 48]) },
        ];
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        let shares: Vec<SecretShare> = encrypted.key_shares.iter().map(|ks| {
