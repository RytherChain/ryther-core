...
use crate::types::{Address, StateKey, U256, keccak256};
...
impl RytherVm {
    ...
    fn execute_inner(
        ...
        self.write_balance(state, &tx.from, tx.sequence_number, new_sender_balance, write_set);
        
        // Increment sender nonce
        self.write_nonce(state, &tx.from, tx.sequence_number, sender_nonce + 1, write_set);
        ...
        self.write_balance(state, &tx.from, tx.sequence_number, final_balance, write_set);
        
        // Pay coinbase
        let coinbase_balance = self.read_balance(state, &block.coinbase, tx.sequence_number, read_set);
        ...
        self.write_balance(state, &block.coinbase, tx.sequence_number, new_coinbase_balance, write_set);
    ...
    fn execute_create(
        ...
        if tx.value != U256::ZERO {
            self.write_balance(state, &contract_address, tx.sequence_number, tx.value, write_set);
        }
    ...
    fn read_balance(&self, state: &MultiVersionState, addr: &Address, seq: u64, read_set: &mut ReadSet) -> U256 {
        state.read_tracked(&Self::balance_key(addr), seq, read_set)
            .unwrap_or(U256::ZERO)
    }
    
    fn read_nonce(&self, state: &MultiVersionState, addr: &Address, seq: u64, read_set: &mut ReadSet) -> u64 {
        state.read_tracked(&Self::nonce_key(addr), seq, read_set)
            .map(|v| v.0[0])
            .unwrap_or(0)
    }
    
    fn write_balance(&self, state: &MultiVersionState, addr: &Address, seq: u64, value: U256, write_set: &mut WriteSet) {
        state.write_tracked(Self::balance_key(addr), seq, Some(value), write_set);
    }
    
    fn write_nonce(&self, state: &MultiVersionState, addr: &Address, seq: u64, nonce: u64, write_set: &mut WriteSet) {
        state.write_tracked(Self::nonce_key(addr), seq, Some(U256::from_u64(nonce)), write_set);
    }
