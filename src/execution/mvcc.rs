...
use crate::types::state::{VersionChain, StateVersion, TxStatus, ReadSet, WriteSet};
    ...
    pub fn finalize(&self) {
        ...
        for entry in self.data.iter() {
            ...
            if let Some((value, _)) = chain.read_for(u64::MAX) {
                match value {
                    Some(v) => { self.base_state.insert(key, v); }
                    None => { self.base_state.remove(&key); }
                }
            }
        }
...
mod tests {
    ...
    fn test_base_state_fallback() {
        let base = vec![
            (make_key(1, 100), U256::from_u64(1000)),
        ];
