...
use super::message::{NetworkMessage, MessageType, EventAnnounce, EventRequest, EventResponse};
...
impl GossipProtocol {
    ...
    pub fn mark_seen(&self, message_id: [u8; 16]) {
        self.seen_messages.write().insert(message_id, Instant::now());
    }
    ...
    pub fn handle_message(&self, msg: &NetworkMessage) -> Option<NetworkMessage> {
        ...
        match &msg.payload {
            ...
            MessageType::EventRequest(request) => {
                self.handle_event_request(&msg.sender, request)
            }
    ...
    fn handle_event_announce(&self, sender: &PeerId, announce: &EventAnnounce) -> Option<NetworkMessage> {
    ...
    fn handle_event_request(&self, _sender: &PeerId, request: &EventRequest) -> Option<NetworkMessage> {
    ...
    pub fn cleanup(&self) {
        ...
        self.seen_messages.write().retain(|_, time| {
            now.duration_since(*time) < self.config.seen_cache_ttl
        });
...
mod tests {
    ...
    fn test_broadcast_event() {
        ...
        let gossip = GossipProtocol::new(local_id, peers.clone(), dag.clone(), GossipConfig::default());
