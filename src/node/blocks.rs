...
use crate::types::block::{Block, TransactionReceipt, Log, TxHash};
...
mod tests {
    ...
    fn test_latest_tracking() {
        ...
        let block1 = Block::from_round(
            1, [0; 32], [1; 32], vec![], vec![], Address::zero(), 0, 100
        );
        store.insert_block(block1);
        assert_eq!(store.latest_number(), 1);
        
        let block2 = Block::from_round(
            2, [0; 32], [2; 32], vec![], vec![], Address::zero(), 0, 200
        );
