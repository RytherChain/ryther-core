        ...
        match node {
            Some(JmtNode::Leaf {
                ...
                if existing_key == *key_hash {
                    ...
                    if existing_bit == new_bit {
                        ...
                        let child_hash =
                            self.put_recursive(&existing_hash, key_hash, value_hash, depth + 1);
            ...
            Some(JmtNode::Internal { left, right }) => {
                ...
                let (new_left, new_right) = if bit {
                    (
                        left,
                        self.put_recursive(&right, key_hash, value_hash, depth + 1),
                    )
                } else {
                    (
                        self.put_recursive(&left, key_hash, value_hash, depth + 1),
                        right,
                    )
                };
