...
async fn test_p2p_peering() {
    // Start node 1 (seed)
    let node1 = TestNode::start(1, false, vec![]).await;
    let node1_addr = format!("127.0.0.1:{}", node1.p2p_port);
    
    // Start node 2 (connects to node 1)
    let node2 = TestNode::start(2, false, vec![node1_addr]).await;
...
async fn test_validator_network() {
    ...
    // Start seed node
    let node1 = TestNode::start(10, true, vec![]).await;
    bootstrap_peers.push(format!("127.0.0.1:{}", node1.p2p_port));
    nodes.push(node1);
    
    // Start 3 other validators
    for i in 1..4 {
        let node = TestNode::start(10 + i, true, bootstrap_peers.clone()).await;
        nodes.push(node);
    }
    
    // Wait for mesh to form (everyone connected to everyone ideally, or at least some)
    let mut mesh_formed = false;
    for _ in 0..200 { // Wait up to 20s
        let min_peers = nodes.iter().map(|n| async {
            n.node.stats().await.connected_peers
        });
    ...
    assert!(mesh_formed, "Validator network failed to form (star topology)");
